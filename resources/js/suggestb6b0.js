...
Suggest.prototype = {
	...
	setListHtml : function(param, listItems){
		...
		if (start > 0) {
			//prev
			htmlElementHead += '<span class="on" id="' + this.idPrefix + 'suggest-' + type + '-prev"' + ' onclick="' + updateFunc + '(\'' + input + '\', ' + prev + ');"><img class="iconA01sl" src="/resources/images/icon_left_b.gif" alt="" height="9" width="3" />Prev 10<\/span>';
		}
		else {
		...
		if (typeof(end) != "undefined" && totalItem > end) {
			//next
			htmlElementHead += '<span class="on" id="' + this.idPrefix + 'suggest-' + type + '-next"' + ' onclick="' + updateFunc + '(\'' + input + '\', ' + next + ');">Next 10<img class="iconA01sr" src="/resources/images/icon_right_b.gif" alt="" height="9" width="3" /><\/span>';
		}
		else {
	...
	updateB7: function(key, start, force){
		...
		if (items.length > 0) {
			...
			for (var j = 0; j < items.length; j++) {
				...
				var linkurl = this.Host + '/product/item/' + b7id + '/';
