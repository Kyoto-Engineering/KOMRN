...
//----------------------------------------------
$.lang_obj = new Object();
$.lang_obj.jp = ['点滅回数','','1回点滅','*回点滅','点滅回数を選択してください。','表示灯','LOCK OUT','点滅している表示灯','エラー内容','原因と対策'];
$.lang_obj.en = ['No. of blinks','Blinks ','1 time','* times','Select the No. of blinks.','Indicator','LOCK OUT','Blinking indicator','Description','Cause and measures'];
$.lang_obj.cn = ['闪烁次数','','闪烁1次','闪烁*次','请选择闪烁次数。','指示灯','LOCK OUT','闪烁的指示灯','错误内容','原因与对策'];
$.lang_obj.it = ['Numero di lampeggi','','1 volta','* volte','Selezionare il numero di lampeggi.','Indicatore','LOCK OUT','Spia lampeggiante','Descrizione','Cause e rimedi'];
$.lang_obj.ko = ['점멸 횟수','','1회 점멸','*회 점멸','점멸 횟수를 선택해 주십시오.','표시등','LOCK OUT','점멸 중인 표시등','에러 내용','원인과 대책'];
$.lang_obj.fr = ['Nº de clignotements','','1 fois','* fois','Sélectionnez le nº de clignotements.','Indicateur','LOCK OUT','Témoin clignotant','Description','Causes et mesures'];
$.lang_obj.de = ['Anzahl','','1Mal','* Mal','Wählen Sie die Anzahl aus','Anzeige','LOCK OUT','Blinkende Anzeige','Beschreibung','Ursache und Maßnahmen'];
$.lang_obj.es = ['N.º de parpadeos','','1 vez','* veces','Seleccione el n.º de parpadeos.','Indicador','LOCK OUT','Indicador que parpadea','Descripción','Causa y medidas'];
...
//----------------------------------------------
//グローバル変数
$.g_pass = "/product/trouble_shooting/f3sg-r/common/img/";
...
function ansChick_select(){
	...
	//alert(ansList.toString()+"  "+slcList.toString());
	...
	if((flg_step==4) && (slcList.toString() != "")){
		slcList_Log[flg_step]=slcList;
		slc_reset(flg_step);
	}else{
				...
				t=t+'<select><option value=""></option>';
					for(var j=1;j<=c;j++){
						if(j==1) {
							...
							t=t+'<option value="'+j+'">'+ timeStr + '</option>';
						}
				}
				t=t+'</select>';
		...
		if(flg_step == 4){
			...
			if(slcList[0].ans[2].length == 1){
				...
				if($.langType=='jp'){
					...
					//その他の言語
					t = t+'<th class="counter">' + $.lang_ary[7] + '</th>';
				}
			}else{
				t =t+ '<th class="counter2" colspan='+slcList[0].ans[2].length+'>' + $.lang_ary[7] + '</th>';
			}
			t=t+'</tr>';
